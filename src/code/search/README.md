...
- bfs를 큐를 이용하여 구현한 예제 코드이다.

```bash
$ python bfsexample.py
1 2 3 8 7 4 5 6 %
```

collections 기본 라이브러리에서 deque 객체를 임포트 해서 큐는 구현하는 것이 빠르다고 했다.<br>
노드와 인접한 노드들에 대한 정보를 2차원 리스트로 나타냈는데 이 방식을 기억해 두고 모든 좌표의 형태를 그래프 형태로 바꿔서 생각하면 좋을 것 같다.<br>
